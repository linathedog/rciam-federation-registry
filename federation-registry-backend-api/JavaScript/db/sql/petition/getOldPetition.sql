SELECT json_build_object('service_name', sd.service_name,'service_description',sd.service_description,
						 ...
						 ,'entity_id',sd.entity_id,'endpoint',sd.endpoint,'pid',sd.pid,'organization_name',sd.name,'organization_url',sd.url,'organization_id',sd.organization_id,'application_type',sd.application_type,
						 'requester',sd.requester,'service_id',sd.service_id,'type',sd.type,'comment',CASE 
							...
							ELSE sd.comment 
							END,'submitted_at',sd.last_edited,'status',sd.status,'reviewed_at',sd.reviewed_at,'aup_uri',sd.aup_uri,
	...
	LEFT JOIN service_petition_details_node USING (id)

SELECT json_build_object('service_name', sd.service_name,'service_description',sd.service_description,'service_type',sd.service_type,
							...
							'access_token_validation_model',sd.access_token_validation_model,
							...
							'requester_info',CASE
								...
								ELSE NULL
								END,
							'service_id',sd.service_id,'type',sd.type,'status',sd.status,'reviewed_at',sd.reviewed_at,'aup_uri',sd.aup_uri,'comment',CASE 
							...
							'reviewer',CASE
								WHEN ${view_reviewer:raw} = TRUE AND sd.reviewer IS NOT NULL THEN
									json_build_object(
										'sub', reviewer_ui.sub,
										'name', reviewer_ui.name,
										'email', reviewer_ui.email
									)
								ELSE NULL
								END,

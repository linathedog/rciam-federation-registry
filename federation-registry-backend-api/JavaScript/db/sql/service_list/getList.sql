SELECT  json_agg(json_build_object('outdated',foo.outdated,'service_id',foo.service_id,'petition_id',foo.petition_id,'service_name',foo.service_name,'service_description',foo.service_description,'logo_uri',foo.logo_uri,'integration_environment',foo.integration_environment,'owned',foo.owned,'status',foo.status,'type',foo.type,'state',foo.state,'group_manager',foo.group_manager,'notification',foo.notification,'comment',
  ...
  SELECT *,COUNT(*) OVER() As full_count,COUNT(case when outdated=true AND petition_id IS NULL AND bar.owned then 1 else null end ) OVER() as outdated_count,COUNT(case when status='request_review' then 1 else null end ) OVER() as request_review_count FROM (
    SELECT DISTINCT service_id,petition_id,service_description,logo_uri,service_name,integration_environment,CASE WHEN owned IS NULL THEN false ELSE owned END,status,type,state,CASE WHEN group_ids.group_manager IS NULL then false ELSE group_ids.group_manager END,CASE WHEN notification IS NULL THEN false ELSE notification END AS notification,comment,service_details.group_id,deployment_type,CASE WHEN petitions.last_edited IS NOT NULL THEN petitions.last_edited ELSE service_state.last_edited END,outdated,client_id,entity_id,pid,created_at,orphan,tags,website_url,country,organizations.name as organization_name
    FROM
    ${select_own_service:raw}
      (SELECT id AS service_id,service_description,logo_uri,service_name,deleted,requester,integration_environment,group_id,website_url,country,organization_id,CASE WHEN (SELECT json_agg((v.sub))
    ...
    LEFT JOIN service_details_node ON service_details.service_id= service_details_node.id
    ${select_all:raw}
    USING (group_id)
    ...
    UNION ALL
    SELECT service_id,petition_id,service_description,logo_uri,service_name,integration_environment,CASE WHEN group_subs.sub=${sub} THEN true ELSE false END AS owned,status,type,state,CASE WHEN group_manager IS NULL THEN false ELSE group_manager END,notification,comment,petitions.group_id,deployment_type,last_edited,outdated,client_id,entity_id,pid,created_at,orphan,tags,website_url,country,organizations.name as organization_name
      FROM
      (SELECT service_id,id AS petition_id,comment,service_description,logo_uri,service_name,integration_environment,CASE WHEN service_petition_details.comment IS NOT NULL THEN true ELSE false END AS notification,status,type,null AS deployment_type,null AS state,group_id,website_url,country,organization_id,last_edited,false as outdated,null::timestamp as created_at,CASE WHEN (SELECT json_agg((v.sub))
      ...
      LEFT JOIN service_petition_details_saml ON petitions.petition_id=service_petition_details_saml.id
      LEFT JOIN service_petition_details_node ON petitions.petition_id=service_petition_details_node.id

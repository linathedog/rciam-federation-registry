...
const amsIngestValidation = () => {
  return [
    body("decoded_messages")
      .exists()
      .withMessage("No agents found")
      .bail()
      .isArray({ min: 1 })
      .withMessage("No agents found")
      .bail()
      .toArray(),
    body("decoded_messages.*.id")
      .exists()
      .withMessage("Required Field")
      .bail()
      .isInt({ gt: 0 })
      .withMessage("Id must be a positive integer"),
    body("decoded_messages.*.deployer_name")
      .optional({ checkFalsy: true })
      .isString()
      .withMessage("Deployer Name must be a String"),
    body("decoded_messages.*.external_id")
      .optional({ checkFalsy: true })
      ...
      .bail(),
    body("decoded_messages.*.client_id")
      .optional({ checkFalsy: true })
      .isString()
      .withMessage("Must be a string")
      .bail()
...
const putAgentValidation = () => {
  return [
    body("type")
      ...
      .bail()
      .custom((value) => {
        if (config.agent.type.includes(value)) {
          return true;
        } else {
          return false;
        }
    ...
    body("entity_type")
      ...
      .bail()
      .custom((value) => {
        if (config.agent.entity_type.includes(value)) {
          return true;
        } else {
          return false;
        }
    ...
    body("integration_environment")
      ...
      .custom((value, { req, location, path }) => {
        if (
          tenant_config[
            req.params.tenant
          ].form.integration_environment.includes(value)
        ) {
          return true;
        } else {
          return false;
      ...
      .withMessage("Integration environment value not supported"),
    body("entity_protocol")
      .exists()
      .withMessage("Required Field")
      .bail()
      .custom((value) => {
        if (config.agent.entity_protocol.includes(value)) {
          return true;
        } else {
          return false;
        }
    ...
    body("hostname")
      ...
      .withMessage("Must be a string")
      .bail()
      .custom((value) => value.match(reg.regSimpleUrl))
...
const getServiceListValidation = () => {
  return [
    ...
    query("protocol")
      ...
      .isString()
      .custom((value, { req, location, path }) => {
        if (tenant_config[req.params.tenant].form.protocol.includes(value)) {
          return true;
        } else {
          return false;
        }
      })
      .withMessage("Protocol not supported"),
    query("owned").optional({ checkFalsy: true }).isBoolean().toBoolean(),
    query("waiting_deployment")
      .optional({ checkFalsy: true })
      .isBoolean()
      .toBoolean(),
    ...
    query("pending_sub")
      ...
      .isString()
      .custom((value, { req, location, path }) => {
        if (["pending", "changes", "request_review"].includes(value)) {
          return true;
        } else {
          return false;
        }
      })
      .withMessage("Value is not supported"),
          ...
          if (value) {
            ...
            if (tags.length > 0) {
              tags.forEach((tag) => {
                if (tag.length > 36) {
                  throw new Error(
                    "The Tag value is not supported because it is too long (" +
                      tag +
                      "), max 36 characters",
                  );
                }
...
const postAgentValidation = () => {
  return [
    body("agents")
      ...
      .bail()
      .toArray(),
    body("agents.*.integration_environment")
      .exists()
      .withMessage("Required Field")
      .isString()
      .custom((value, { req, location, path }) => {
...
const postBannerAlertValidation = () => {
  return [
    param("tenant")
      .custom((value, { req, location, path }) => {
        if (value in tenant_config) {
          return true;
        } else {
          return false;
        }
      })
      .withMessage("Invalid Tenant in the url"),
    body("alert_message")
      .exists()
      .bail()
      .isString()
      .withMessage("alert_message must be a string")
      .isLength({ min: 4, max: 1024 })
      .withMessage("alert_message must be from 4 to 1024 characters long"),
    body("type")
      .exists()
      .bail()
      .custom((value) => {
        ...
        if (supported_types.includes(value)) {
          ...
          throw new Error(
            value +
              " is not a supported Banner Alert type. Supported types: (" +
              supported_types +
              ")",
          );
        }
    ...
    body("priority")
      .optional()
      .isInt()
      .withMessage("priority must must an integer"),
...
const getServicesValidation = () => {
  return [
    query("integration_environment")
      ...
      .withMessage("integration_environment value not supported"),
    query("protocol")
      .optional({ checkFalsy: true })
      .isString()
      .custom((value, { req, location, path }) => {
      ...
      .withMessage("protocol value not supported"),
    query("protocol_id")
      .optional({ checkFalsy: true })
      .isString()
      .withMessage("protocol_id must be a string")
      .if((value) => {
        return value.constructor === stringConstructor;
      })
      .isLength({ min: 2, max: 128 })
      .withMessage("protocol_id must be between 2 and 128 characters"),
    param("tenant")
      .custom((value, { req, location, path }) => {
        if (value in tenant_config) {
          return true;
...
const serviceValidationRules = (options, req) => {
  ...
  const requiredIntegrationEnvironment = (
    ...
    let error = false;
    if (
      tenant_config[tenant].form.more_info[field]?.required?.includes(env) &&
      isEmpty(value)
    ) {
      if (options.optional) {
        req.body[pos].outdated = true;
        append_error(
          ...
          pos,
          field,
          field + " is a required attribute for the " + env + " environment",
        );
        error = false;
      } else {
        error = true;
  ...
  return [
    body()
      .isArray({ min: 1 })
      .withMessage("Body must be an array containing at least one service"),
    body("*.tenant")
      .custom((value, { req, location, path }) => {
        if (
          options.tenant_param ||
          req.body[path.match(/\[(.*?)\]/)[1]].tenant in tenant_config
        ) {
          return true;
        } else {
          return false;
    ...
    body("*.service_name")
      .custom((value, { req, location, path }) => {
        return required(value, req, path.match(/\[(.*?)\]/)[1], "service_name");
      })
      .withMessage("Service name missing")
      ...
      .withMessage("Service name must be from 2 up to 256 characters"),
    body("*.country")
      .custom((value, { req, location, path }) => {
        ...
        let general_requirement = !required(
          value,
          req,
          path.match(/\[(.*?)\]/)[1],
          "country",
        );
        let tenant_requirement = tenant_config[
          tenant
        ].form.more_info.country.required.includes(integration_environment);
    ...
    body("*.service_description")
      ...
      .withMessage("Service Description missing")
      .if((value) => {
        return value;
      })
      .trim()
      ...
      .isLength({ min: 1 })
      .withMessage("Service description can't be empty"),
    body("*.logo_uri")
      .optional({ checkFalsy: true })
      .trim()
      .isString()
      .withMessage("Service Logo must be a string")
      .custom((value) => value.match(reg.regUrl))
      .withMessage("Service Logo must be a secure url https://")
      .isLength({ max: 256 })
      .withMessage("Service logo cant exceed character limit (6000)"),
    body("*.policy_uri")
      .custom((value, { req, location, path }) => {
      ...
      .withMessage("Service Policy Uri missing")
      .if((value) => {
        return value;
      })
      .trim()
      .isString()
      .withMessage("Service Policy Uri must be a string")
      .custom((value) => value.match(reg.regSimpleUrl))
    ...
    body("*.requested_attributes")
      ...
      .custom((attributes, { req, location, path }) => {
        try {
          attributes.map((attribute, index) => {
            if (
              !defaultAttributes.some(
                (e) => e.friendly_name === attribute.friendly_name,
              )
            ) {
              throw new Error(
                "Attribute not supported (" + attribute.firendly_name + ")",
              );
            }
            if (attribute.name.length > 0) {
              throw new Error(
                "Attribute name must be at least 1 character (" +
                  attribute.firendly_name +
                  ")",
              );
            }
            if (attribute.name.length >= 512) {
              throw new Error(
                "Attribute name exceeds character limit (512) :(" +
                  attribute.firendly_name +
                  ")",
              );
            }
            if (typeof attribute.required !== "boolean") {
              throw new Error(
                "Attribute required property must be boolean :(" +
                  attribute.firendly_name +
                  ")",
              );
            }
            if (
              attribure.name_format !==
              "urn:oasis:names:tc:SAML:2.0:attrname-format:uri"
            ) {
              throw new Error(
                "Attribute Name Format property must be 'urn:oasis:names:tc:SAML:2.0:attrname-format:uri' :(" +
                  attribute.firendly_name +
                  ")",
              );
            }
    ...
    body("*.contacts")
      .custom((value, { req, location, path }) => {
        return required(value, req, path.match(/\[(.*?)\]/)[1], "contacts");
      })
      .withMessage("Service Contacts missing")
      .if((value) => {
        return value && Array.isArray(value) && value.length !== 0;
      })
      .isArray({ min: 1 })
      .withMessage("Service Contacts must be an array")
      .custom((value, { req, location, path }) => {
        let tenant = options.tenant_param
          ? req.params.tenant
          : req.body[path.match(/\[(.*?)\]/)[1]].tenant;
        ...
        try {
          value.map((contact, index) => {
            if (
              (contact.email &&
                !contact.email.toLowerCase().match(reg.regEmail)) ||
              !tenant_config[tenant].form.contact_types.includes(contact.type)
            ) {
              throw new Error("Invalid contact format");
            }
          });
          tenant_config[tenant].form.contact_requirements.forEach(
            (requirement, index) => {
    ...
    body("*.protocol")
      ...
      .custom((value, { req, location, path }) => {
        let tenant = options.tenant_param
          ? req.params.tenant
          : req.body[path.match(/\[(.*?)\]/)[1]].tenant;
        if (tenant_config[tenant].form.protocol.includes(value)) {
          return true;
        } else {
          return false;
        }
      })
      .withMessage("Invalid Protocol value"),
    body("*.client_id")
      .if((value, { req, location, path }) => {
        let skip;
      ...
      .exists({ checkFalsy: true })
      .withMessage("client_id is missing")
      .if((value) => {
        return value;
      })
      .trim()
      ...
      .isLength({ min: 2, max: 128 })
      .withMessage("client_id must be between 2 and 128 characters")
      .custom((value, { req, location, path }) => {
        try {
    ...
    body("*.redirect_uris")
      .custom((value, { req, location, path }) => {
        ...
        if (Array.isArray(req.body[pos].grant_types)) {
          grant_types_bool =
            req.body[pos].grant_types.includes("implicit") ||
            req.body[pos].grant_types.includes("authorization_code");
        } else {
        ...
        try {
          if (Array.isArray(value) && value.length > 0) {
            value.map((item, index) => {
              ...
              let isLocalIp = false;
              if (
                tenant_config[tenant].test_env.includes(
                  req.body[pos].integration_environment,
                )
              ) {
                isLocalIp =
                  item.match(reg.regIpv4Local) || item.match(reg.regIpv6Local);
              }
              if (!isLocalIp) {
                try {
                  url = new URL(item);
      ...
      .custom((value, { req, location, path }) => {
        ...
        try {
          if (Array.isArray(value) && value.length > 0) {
            ...
            throw new Error(
              "Service post_logout_redirect_uris must be an array",
            );
          }
          return true;
        } catch (err) {
    ...
    body("*.grant_types")
      .if((value, { req, location, path }) => {
        return (
          value && req.body[path.match(/\[(.*?)\]/)[1]].protocol === "oidc"
        );
      })
      .isArray()
      .withMessage("grant_types must be an array")
      .custom((value, { req, location, path }) => {
        let success = true;
        let tenant = options.tenant_param
          ? req.params.tenant
        ...
        try {
          value.map((item, index) => {
            if (!tenant_config[tenant].form.grant_types.includes(item)) {
              //reuse_refresh_token(item);
              success = false;
            }
          });
    ...
    body("*.jwks_uri")
      .customSanitizer((value, { req, location, path }) => {
        if (
          req.body[path.match(/\[(.*?)\]/)[1]].token_endpoint_auth_method ===
          "private_key_jwt"
        ) {
          return value;
        } else {
      ...
      .custom((value, { req, location, path }) => {
        if (req.body[path.match(/\[(.*?)\]/)[1]].jwks && value) {
          throw new Error(
            "Invalid private key option, only one of the following values should exist ['jwks','jwks_uri']",
          );
        } else if (!(req.body[path.match(/\[(.*?)\]/)[1]].jwks || value)) {
          throw new Error(
            "Invalid private key option, one of the following values should exist ['jwks','jwks_uri']",
          );
    ...
    body("*.jwks")
      ...
      .if((value, { req, location, path }) => {
        return (
          req.body[path.match(/\[(.*?)\]/)[1]].token_endpoint_auth_method ===
            "private_key_jwt" &&
          !req.body[path.match(/\[(.*?)\]/)[1]].jwks_uri &&
          value
        );
      })
      .custom((value) => {
    ...
    body("*.application_type")
      ...
      .withMessage("Service application_type is missing")
      .if((value, { req, location, path }) => {
        return (
          value && req.body[path.match(/\[(.*?)\]/)[1]].protocol === "oidc"
        );
      ...
      .custom((value, { req, location, path }) => {
        ...
        if (
          !value ||
          tenant_config[tenant].form.application_type.includes(value)
        ) {
          return true;
        } else {
          return false;
    ...
    body("*.token_endpoint_auth_method")
      ...
      .withMessage("Service token_endpoint_auth_method missing")
      .if((value, { req, location, path }) => {
        return (
          value && req.body[path.match(/\[(.*?)\]/)[1]].protocol === "oidc"
        );
      ...
      .custom((value, { req, location, path }) => {
        ...
        if (
          !value ||
          tenant_config[tenant].form.token_endpoint_auth_method.includes(value)
        ) {
          return true;
        } else {
          return false;
    ...
    body("*.token_endpoint_auth_signing_alg")
      .customSanitizer((value, { req, location, path }) => {
        if (
          !["private_key_jwt", "client_secret_jwt"].includes(
            req.body[path.match(/\[(.*?)\]/)[1]].token_endpoint_auth_method,
          )
        ) {
          return "";
      ...
      .if((value, { req, location, path }) => {
        return ["private_key_jwt", "client_secret_jwt"].includes(
          req.body[path.match(/\[(.*?)\]/)[1]].token_endpoint_auth_method,
        );
      })
      .custom((value, { req, location, path }) => {
      ...
      .custom((value, { req, location, path }) => {
        return tenant_config[
          req.params.tenant
        ].form.token_endpoint_auth_signing_alg.includes(value);
      })
      .withMessage("Invalid Token Endpoint Signing Algorithm"),
    body("*.id_token_timeout_seconds")
      .customSanitizer((value) => {
    ...
    body("*.refresh_token_validity_seconds")
      .customSanitizer((value, { req, location, path }) => {
        const pos = path.match(/\[(.*?)\]/)[1];
        const scopes = req.body[pos].scope;

        const hasOfflineAccess =
          Array.isArray(scopes) && scopes.includes("offline_access");
      ...
      .custom((value, { req, location, path }) => {
        ...
        if (!hasOfflineAccess) {
          req.body[pos].refresh_token_validity_seconds = null;
          return true;
        }
      ...
      .if((value, { req, location, path }) => {
        ...
        return Array.isArray(scopes) && scopes.includes("offline_access");
      })
      .custom((value, { req, location, path }) => {
        if (isEmpty(value)) {
          return true;
    ...
    body("*.device_code_validity_seconds")
      ...
      .custom((value, { req, location, path }) => {
        ...
        if (
          isEmpty(value) &&
          req.body[pos].protocol === "oidc" &&
          req.body[pos].grant_types &&
          req.body[pos].grant_types.includes(
          ...
          if (options.optional) {
            ...
            throw new Error(
              "Device Code Validity Seconds is required when 'urn:ietf:params:oauth:grant-type:device_code' is included in the grant_types",
            );
          }
        }
        let tenant = options.tenant_param
        ...
        if (isEmpty(value)) {
          ...
          (isNotEmpty(value) && value <= max && value >= 0) ||
          req.body[pos].protocol !== "oidc" ||
          isEmpty(req.body[pos].grant_types) ||
          !req.body[pos].grant_types.includes(
            "urn:ietf:params:oauth:grant-type:device_code",
          )
        ) {
          ...
          throw new Error(
            "Device Code Validity Seconds must be an integer in specified range [0-" +
              max +
              "]",
          );
        }
    ...
    body("*.code_challenge_method")
      ...
      .custom((value, { req, location, path }) => {
        try {
          let pos = path.match(/\[(.*?)\]/)[1];
          if (
            req.body[pos].token_endpoint_auth_method === "none" &&
            req.body[pos].grant_types.includes("authorization_code") &&
            !value
          ) {
            return false;
    ...
    body("*.allow_introspection")
      ...
      .withMessage("Allow introspection missing")
      .if((value, { req, location, path }) => {
        return (
          value && req.body[path.match(/\[(.*?)\]/)[1]].protocol === "oidc"
        );
      ...
      .custom((value) => typeof value === "boolean")
      .withMessage("Allow introspection must be a boolean")
      .bail(),
    body("*.generate_client_secret")
      .optional({ checkFalsy: true })
      .custom((value) => typeof value === "boolean")
      .withMessage("Generate client secret must be a boolean"),
    body("*.reuse_refresh_token")
      .customSanitizer((value) => {
        if (typeof value !== "boolean") {
          return false;
        } else {
      ...
      .custom((value) => typeof value === "boolean")
      .withMessage("Reuse refresh tokens must be a boolean"),
    body("*.integration_environment")
      .exists({ checkFalsy: true })
      .withMessage("Integration Environment missing")
      .if((value) => {
        return value;
      })
      .custom((value, { req, location, path }) => {
        let tenant = options.tenant_param
          ...
          : req.body[path.match(/\[(.*?)\]/)[1]].tenant;
        if (
          tenant_config[tenant].form.integration_environment.includes(value)
        ) {
          return true;
        } else {
          return false;
      ...
      .withMessage("Invalid Integration Environment"),
    body("*.clear_access_tokens_on_refresh")
      .customSanitizer((value) => {
        if (typeof value !== "boolean") {
          return false;
        } else {
      ...
      .custom((value) => typeof value === "boolean")
      .withMessage("Clear access tokens on refresh must be a boolean")
      .bail(),
    body("*.client_secret")
      .customSanitizer((value, { req, location, path }) => {
        if (
          (options.sanitize &&
            req.body[path.match(/\[(.*?)\]/)[1]].protocol !== "oidc") ||
          ["none", null, "private_key_jwt"].includes(
            req.body[path.match(/\[(.*?)\]/)[1]].token_endpoint_auth_method,
          )
        ) {
          return null;
      ...
      .if((value, { req, location, path }) => {
        return (
          (value ||
            !req.body[path.match(/\[(.*?)\]/)[1]].generate_client_secret) &&
          req.body[path.match(/\[(.*?)\]/)[1]].protocol === "oidc" &&
          [
            "client_secret_basic",
            "client_secret_post",
      ...
      .exists({ checkFalsy: true })
      .withMessage("Client secret is missing")
      .if((value) => {
        return value;
      })
      .trim()
    ...
    body("*.entity_id")
      ...
      .withMessage("Entity ID is missing")
      .if((value, { req, location, path }) => {
        return (
          value && req.body[path.match(/\[(.*?)\]/)[1]].protocol === "saml"
        );
      })
      .trim()
      .isString()
        ...
        try {
          if (value.constructor === stringConstructor) {
            if (value.length < 4 || value.length > 256) {
              throw new Error(
                "Entity id is not in specified character length range (4-256)",
              );
            }
          }
    ...
    body("*.metadata_url")
      .if((value, { req, location, path }) => {
        return req.body[path.match(/\[(.*?)\]/)[1]].protocol === "saml";
      })
      .exists({ checkFalsy: true })
      .withMessage("Metadata url missing")
      .if((value) => {
        return value;
      })
      .trim()
      .isString()
      .withMessage("Metadata url must be a string")
      .if((value) => {
        return value.constructor === stringConstructor;
      })
      .custom((value) => {
      ...
      .withMessage("Metadata url must be a url")
      .if(() => {
        return options.check_available;
      })
      .custom((value, { req, location, path }) => {
        ...
        if (isPolicyField && requiredForEnvironment && applicable) {
          ...
          if (!fieldEnabled) {
            optionalError(
              ...
              pos,
              extra_field,
              extra_field + " field should be enabled",
            );
          }
        }
      }
    ...
    body("*.organization_id")
      .customSanitizer((value, { req, location, path }) => {
        ...
        if (
          (
            tenant_config[tenant]?.form.extra_fields?.organization?.hide || []
          ).includes(integration_environment)
        ) {
          return null;
        } else {
          return value;
      ...
      .custom((value, { req, location, path }) => {
        ...
        let extra_fields = tenant_config[tenant].form.extra_fields;
        if (
          !extra_fields.organization.required.includes(integration_environment)
        ) {
          return true;
        } else {
          if (isEmpty(value)) {
...
const petitionValidationRules = () => {
  ...
  return [
    body("service_id")
      .if(
        body("type").custom((value) => {
          return value === "edit" || values === "delete";
        }),
      )
      .exists()
      .withMessage("Required Field")
    ...
    body("type")
      ...
      .bail()
      .custom((value) => {
        if (["edit", "create", "delete"].includes(value)) {
          return true;
        } else {
          return false;
        }
...
const decodeAms = (req, res, next) => {
  try {
    ...
    req.body.messages.forEach((item) => {
      req.body.decoded_messages.push(
  ...
  if (!errors.isEmpty()) {
    ...
    errors.errors.forEach((error, index) => {
      ...
      if (typeof parseInt(matches[1]) == "number") {
        !service_ids.includes(req.body[matches[1]].id) &&
          service_ids.push(req.body[matches[1]].id);
    ...
    fs.appendFileSync(
      logFile,
      util.inspect(errors.errors, { maxArrayLength: null }),
    );
        ...
        if (typeof service === "object" && service !== null) {
          ...
          for (const extra_field in extra_fields) {
            if (
              ...
              req.body[index].service_boolean[extra_field] =
                req.body[index][extra_field] === "true" ||
                req.body[index][extra_field] === true
                  ? true
                  : false;
            }

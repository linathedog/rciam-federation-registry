    ...
    <React.Fragment>
      <InputGroup>
        <Form.Control
          ...
          className={
            props.changed
              ? "col-form-label.sm input-edited"
              : "col-form-label.sm"
          }
        />
        {props.copybutton ? (
          <CopyToClipboardComponent value={props.value} />
        ) : null}
      </InputGroup>
        ...
        <InputGroup.Prepend>
          {!props.disabled ? (
            <OverlayTrigger
              placement="right"
              overlay={
                <Tooltip id={`tooltip-right`}>Load Metadata from Url</Tooltip>
              }
            >
              <Button
                disabled={
...
export function OrganizationField(props) {
  ...
  const [singleSelections, setSingleSelections] = useState(
    props.values.organization_name ? [props.values.organization_name] : []
  );
            ...
            .then((response) => {
              if (response) {
                ...
                response.organizations.forEach((item, index) => {
                  ...
                  if (
                    item.organization_name === singleSelections[0] &&
                    options[item.organization_name].url
                  ) {
                    loaded = true;
                  }
                });
                if (ror_response && ror_response.items) {
                  ror_response.items.forEach((item, index) => {
                    let name =
                      item?.names?.find((n) => n.lang === "en")?.value ||
                      item?.names[0].value;
                    ...
                    options[name].url =
                      item?.links?.find((n) => n.type === "website")?.value ||
                      item?.links[0]?.value;
                    options[name].ror_id = item.id;
                    if (
                      name +
                ...
                if (!exists && searchString.length > 0) {
                  ...
                  newOption[searchString + " (Add New Organization)"].name =
                    searchString;
                  newOption[searchString + " (Add New Organization)"].url =
                    null;
                }
                setOrganizations({
                  ...(newOption ? newOption : []),
  ...
  const handleChange = async (selected, index) => {
    let organization = organizations[selected[0]]
      ? organizations[selected[0]]
      : {};
    ...
    props
      .setFieldValue(
        "organization_name",
        selected && selected[0] ? organizations[selected[0]].name : "",
        false
      )
      .then(() => {
    ...
    props
      .setFieldValue(
        "organization_url",
        organizations[selected[0]] && organizations[selected[0]].url
          ? organizations[selected[0]].url
          : selected[0] && selected[0].contains(" (Add New Organization)")
          ? props.values.organization_url
          : "",
        false
      )
      .then(() => {
      ...
      <Form.Group className="organizations-input">
        <InputGroup>
          ...
          <Typeahead
            ...
            onBlur={() => {
              props.setFieldTouched("organization_name");
            }}
            onInputChange={(e) => {
              getOrganizations(e);
            }}
...
export function AuthMethRadioList(props) {
  ...
  useEffect(() => {
    if (
      (props.values.token_endpoint_auth_method === "client_secret_jwt" ||
    ...
    <React.Fragment>
      {props.radio_items.map((item, index) => (
        <div
          className={
            "form_radio_item " +
            (props.changed && props.values.token_endpoint_auth_method === item
              ? "input_radio_edited"
              : null)
          }
          key={index}
          onMouseOver={() => {
            props.values.token_endpoint_auth_method === item && setShow(true);
          }}
          onMouseOut={() => {
            props.values.token_endpoint_auth_method === item && setShow(false);
          }}
        >
          <Field name={props.name}>
            {({ field, form }) => (
              <React.Fragment>
                <input
                  ...
                  value={item}
                  ref={
                    props.values.token_endpoint_auth_method === item
                      ? target
                      : null
                  }
      ...
      >
        <Form.Check
          ...
          className={
            ...
            (props.changed ? "input-edited checkbox-edited" : "") +
            (props.changed && props.moreinfo
    ...
    <React.Fragment>
      <Col sm="4">
        <Form.Control
          ...
          type="text"
          className={
            "col-form-label-sm " + (props.changed ? "input-edited" : "")
          }
          ref={target}
...
export function CountrySelect(props) {
  ...
  return (
    <React.Fragment>
      <div
        className={
          "select-container" + (props.changed ? " input-edited" : null)
        }
        ref={target}
      >
        <Field
        ...
        >
          {[{ countryName: "Select your country..." }, ...countryData].map(
            (country, index) => (
              <option
                key={index}
                value={
                  index === 0 ? null : country.countryShortCode.toLowerCase()
                }
              >
                {country.countryName}
              </option>
      ...
      >
        ...
        >
          {props.options.map((item, index) => (
            <option key={index} value={props.options[index]}>
              {props.optionsTitle[index]}
            </option>
          ))}
        </Field>
        ...
        {props.copybuttonActive ? (
          <OverlayTrigger
            placement="right"
            overlay={<Tooltip id={`tooltip-right`}>Copy Node</Tooltip>}
          >
            <Button
        ...
        >
          {props.options.map((item, index) => (
            <option
              ...
              disabled={props.disabled_option === props.options[index]}
            >
              {props.optionsTitle[index]}
            </option>
          ))}
...
function CheckboxListItem(props) {
  ...
  return (
    <tr
      ...
      onMouseOut={() => setShow(false)}
    >
      <td className={props.class}>{props.item}</td>
      <MyOverLay
        show={props.class && show}
        type={
          props.class === "row-deleted"
            ? "Deleted"
            : props.class === "row-added"
            ? "Added"
            : null
        }
        target={target}
...
export function CheckboxList(props) {
  ...
  return (
    <React.Fragment>
      {props.changed &&
      (existing.length > 0 || added.length > 0 || deleted.length > 0) ? (
        <Table className="client-list-table">
          <thead>
            {existing.map((item, index) => {
              return <CheckboxListItem key={index} item={item} />;
            })}
              ...
              return (
                <div className="checkboxList" key={index}>
                  ...
                  (item.substr(0, 33) === "urn:ietf:params:oauth:grant-type:" ||
                    item.substr(0, 33) === "urn:ietf:params:oauth:grant_type:")
                    ? item.substr(33).replace("_", " ")
                    : item.replace("_", " ")}
                  {props.deprecated_options.includes(item)
                    ? " (deprecated)"
                    : ""}
                </div>
              );
    ...
    <React.Fragment>
      <div
        className={
          "checkbox-item " +
          (props.changed &&
          (props.changed.scope.D?.includes("offline_access") ||
            props.changed.scope.N?.includes("offline_access"))
            ? "input-edited checkbox-item-edited"
            : "")
        }
        ref={target}
        onMouseOver={() => setShow(true)}
          ...
          >
            {!tenant.form_config.disabled_fields.includes(
              ...
              <SimpleCheckbox
                ...
                changed={
                  props.changed
                    ? props.changed.clear_access_tokens_on_refresh
                    : null
                }
                onChange={props.onChange}
                disabled={props.disabled}
    ...
    <React.Fragment>
      <div
        className={
          "checkbox-item " +
          (props.changed &&
          (props.changed?.grant_types?.D?.includes(
            "urn:ietf:params:oauth:grant-type:device_code"
          ) ||
            props.changed?.grant_types?.N?.includes(
              "urn:ietf:params:oauth:grant-type:device_code"
            ))
            ? "input-edited checkbox-item-edited"
            : "")
        ...
        <React.Fragment>
          ...
          <InputGroup>
            <Form.Control
              ...
              className={
                (editSecret
                  ? "d-none col-form-label-sm"
                  : "col-form-label-sm") +
                (props.changed ? " input-edited" : "")
              }
              onChange={props.onChange}
              isInvalid={props.isInvalid}
              onBlur={props.onBlur}
              ...
              onMouseOut={() => setShow(false)}
            />
            {props.copybutton && !editSecret ? (
              <CopyToClipboardComponent value={props.client_secret} />
            ) : null}
          </InputGroup>
...
export function ListInputArray(props) {
  ...
  return (
    <Table
      ...
      className={
        "input-list-table" +
        (props.disabled ? " input-list-table-disabled" : "")
      }
    >
      <thead>
        {!props.disabled ? (
                    ...
                    {Array.isArray(props.error) ? (
                      <tr>
                        <td className="error-td">
                          <div className="error-message-list-item">
                            {props.error[index]}
                          </div>
                        </td>
                        <td></td>
...
function ListInputArrayInput1(props) {
  ...
  return (
    <React.Fragment>
      ...
      <MyOverLay
        show={type && type !== "Exists" && show ? "true" : ""}
        type={type}
        target={target}
      />
    </React.Fragment>
...
function ListInputArrayInput2(props) {
  ...
  return (
    <tr
      className={
        (Array.isArray(props.error) || typeof props.error === "string") &&
        props.error[props.index]
          ? "error-tr"
          : null
      }
      onMouseOver={() => setShow(true)}
  ...
  return (
    <React.Fragment>
      <Form.Control
        ...
        name={props.name}
        className={
          "col-form-label-sm " + (props.changed ? "input-edited" : null)
        }
        placeholder="https//"
        value={props.value}
        onBlur={props.onBlur}
      ...
      {props.description || (props.moreInfo && props.moreInfo.description) ? (
        <Form.Text className="text-muted text-left">
          {parse(
            props.moreInfo && props.moreInfo.description
      ...
      {props.warning && !props.error ? (
        <div className="warning-message">
          {" "}
          <FontAwesomeIcon icon={faExclamationTriangle} />
          Warning: Image could not be loaded, make sure the url points to an
          image resourse
        </div>
      ) : null}
      {props.error && props.touched ? (
        typeof props.error === "string" ? (
          ...
          <div className="error-message">{t("input_image_error")}</div>
        )
      ) : null}
      <FormikConsumer>
          ...
          >
            <Image
              ...
              src={
                props.value
                  ? props.value
                  : process.env.PUBLIC_URL + "/logo_placeholder.gif"
              }
  ...
  return (
    ...
    >
      {(propsOv) => (
        <Tooltip
          id="overlay-example"
          placement={propsOv.placement}
          arrowProps={propsOv.arrowProps}
          ref={propsOv.ref}
          style={propsOv.style}
          outofboundaries={propsOv.outofboundaries}
        >
          {props.type === "Added"
            ? t("input_added")
            : props.type === "Deleted"
  ...
  return (
    <React.Fragment>
      <Form.Control
        ...
        ref={target}
        isInvalid={
          Array.isArray(props.error) ? !!props.error[props.index] : false
        }
        column="true"
        sm="4"
        type="text"
        className={
          "col-form-label.sm " +
          (type === "Added" ? "input-new" : !type ? "" : "input-deleted")
        }
        placeholder="https//"
        disabled={props.disabled}
      />
      <MyOverLay
        show={type && show ? "string" : null}
        type={type}
        target={target}
      />
    </React.Fragment>
      ...
      {({ push, remove, insert }) => (
        <React.Fragment>
          {!props.disabled || !props.values ? (
            <InputGroup
              className={
                props.empty && props.touched ? "invalid-input mb-3" : "mb-3"
              }
            >
              <Form.Control
                value={newVal}
                onChange={(e) => {
                  setNewVal(e.target.value.trim());
                }}
                column="true"
                sm="4"
                onBlur={() => {
                  !props.touched && props.setFieldTouched(props.name, true);
                }}
                type="text"
                className="col-form-label.sm"
                placeholder={props.placeholder}
          ...
          {((props.values && props.values.length > 0) || props?.changed?.D) &&
            [
              ...props.values,
              ...(props?.changed?.D ? props.changed?.D : []),
            ].map((item, index) => (
          ...
          {props.values &&
            ...
            [
              ...
              <React.Fragment key={index}>
                ...
                <div className="error-message-list-item">
                  {Array.isArray(props.error) && props.error[index]
                    ? props.error[index].email
                    : ""}
                </div>
              </React.Fragment>
            ))}
        ...
        <React.Fragment>
          <Form.Control
            ...
            className={
              "col-form-label.sm " +
              (type === "Added"
                ? "input-new remove-border-right"
                : type === "Deleted"
                ? "input-deleted remove-border-right"
                : null)
            }
                ...
                <React.Fragment>
                  <Form.Control
                    ...
                    className={
                      "input-hide  " +
                      (type === "Added"
                        ? "input-new remove-border-left"
                        : type === "Deleted"
                        ? "input-deleted remove-border-left"
                        : null)
                    }
                    onBlur={props.handleBlur}
                    onChange={props.onChange}
